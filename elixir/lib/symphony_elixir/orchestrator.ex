defmodule SymphonyElixir.Orchestrator do
  ...
  defp restart_stalled_issue(state, issue_id, running_entry, now, timeout_ms) do
    ...
    if is_integer(elapsed_ms) and elapsed_ms > timeout_ms do
      ...
      if input_required_blocker?(running_entry) do
        error = blocker_error(running_entry, "stalled for #{elapsed_ms}ms after Claude requested operator input")
  ...
  defp input_required_blocker?(running_entry) when is_map(running_entry) do
    Map.get(running_entry, :last_claude_event) in [:turn_input_required, :approval_required] or
      not is_nil(input_required_completion_outcome(Map.get(running_entry, :completion))) or
      claude_message_method(Map.get(running_entry, :last_claude_message)) ==
  ...
  defp blocker_error(running_entry, fallback) when is_map(running_entry) do
    claude_event_blocker_error(Map.get(running_entry, :last_claude_event)) ||
      completion_blocker_error(Map.get(running_entry, :completion)) ||
      claude_message_blocker_error(Map.get(running_entry, :last_claude_message)) ||
      fallback
  end

  defp blocker_error(_running_entry, fallback), do: fallback

  defp claude_event_blocker_error(:turn_input_required), do: "claude turn requires operator input"
  defp claude_event_blocker_error(:approval_required), do: "claude turn requires approval"
  defp claude_event_blocker_error(_event), do: nil

  defp completion_blocker_error(completion) do
    case input_required_completion_outcome(completion) do
      outcome when outcome in [:input_required, :needs_input] -> "claude turn requires operator input"
      :approval_required -> "claude turn requires approval"
      nil -> nil
    end
  end

  defp claude_message_blocker_error(message) do
    if claude_message_method(message) == "mcpServer/elicitation/request" do
      "claude MCP elicitation requires operator input"
    end
  end

  defp claude_message_method(%{message: %{"method" => method}}) when is_binary(method), do: method
  defp claude_message_method(%{message: %{method: method}}) when is_binary(method), do: method
  defp claude_message_method(%{"method" => method}) when is_binary(method), do: method
  defp claude_message_method(%{method: method}) when is_binary(method), do: method
  defp claude_message_method(_message), do: nil
  ...
  defp block_issue_from_entry(%State{} = state, issue_id, running_entry, error) do
    blocked_entry = %{
      ...
      last_claude_message: Map.get(running_entry, :last_claude_message),
      last_claude_event: Map.get(running_entry, :last_claude_event),
      last_claude_timestamp: Map.get(running_entry, :last_claude_timestamp)
    }
    ...
    blocked =
      ...
      |> Enum.map(fn {issue_id, metadata} ->
        %{
          ...
          last_claude_timestamp: Map.get(metadata, :last_claude_timestamp),
          last_claude_message: Map.get(metadata, :last_claude_message),
          last_claude_event: Map.get(metadata, :last_claude_event)
        }
      end)
